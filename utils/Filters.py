...
def cartoon(baseString):
    ...
    # Convert img_edge to BGR format before bitwise_and operation
    img_edge = cv2.cvtColor(img_edge, cv2.COLOR_GRAY2BGR)
    
    # Ensure that img and img_edge have the same dimensions
    img_edge = cv2.resize(img_edge, (img.shape[1], img.shape[0]))
    ...
    _, img_encoded = cv2.imencode('.png', final)
    img_code = base64.b64encode(img_encoded).decode('utf-8')
    
    print(sys._getframe().f_code.co_name + " finished in " + "{:.2f}".format(time.time()-start)+" s")
    return img_code
